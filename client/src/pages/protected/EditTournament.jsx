...
const EditTournament = () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          setFormData({
            title: t.title || '',
            game: t.game || '',
            bracketType: t.bracketType || 'single-elimination',
            maxTeams: t.maxTeams || 16,
            playersPerTeam: t.playersPerTeam || 5,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await expressApi.put(`/api/tournaments/${id}`, payload);

      if (res.data.success) {
        navigate(`/tournaments/${id}`); // Redirect back to details page
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to permanently delete this tournament? This action cannot be undone.')) {
      try {
        setLoading(true);
        const res = await expressApi.delete(`/api/tournaments/${id}`);
        if (res.data.success) {
          navigate('/dashboard');
        }
      } catch (err) {
