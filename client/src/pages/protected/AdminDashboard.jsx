  ...
  return (
    <div className="container py-8 sm:py-12 min-h-[calc(100vh-80px)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <Link to="/tournaments?status=completed" className="bg-surface border border-border rounded-[8px] p-6 shadow-sm flex items-center justify-between transition-colors hover:border-primary group block cursor-pointer">
          <div>
            <p className="text-[0.8rem] text-text-secondary uppercase font-bold tracking-widest mb-1 group-hover:text-primary transition-colors">Completed Tournaments</p>
            <p className="text-[2rem] font-bold text-text leading-none">{stats.completedTournaments || 0}</p>
          </div>
          <div className="w-12 h-12 bg-accent/10 rounded-full flex items-center justify-center text-accent text-xl group-hover:bg-accent group-hover:text-white transition-all">
            <i className="fa-solid fa-trophy"></i>
          </div>
        </Link>
        <Link to="/tournaments?status=open" className="bg-surface border border-border rounded-[8px] p-6 shadow-sm flex items-center justify-between transition-colors hover:border-primary group block cursor-pointer">
          <div>
            <p className="text-[0.8rem] text-text-secondary uppercase font-bold tracking-widest mb-1 group-hover:text-primary transition-colors">Active Events</p>
            <p className="text-[2rem] font-bold text-text leading-none">{stats.activeTournaments}</p>
          </div>
          <div className="w-12 h-12 bg-emerald-500/10 rounded-full flex items-center justify-center text-emerald-500 text-xl group-hover:bg-emerald-500 group-hover:text-white transition-all">
              ...
              <tbody className="divide-y divide-border">
                {filteredUsers.map(u => (
                  <tr key={u._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded text-[0.75rem] font-bold uppercase tracking-widest ${u.role === 'admin' ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30' :
                ...
                {reports.map(r => (
                  <tr key={r._id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 max-w-xs">
                      <p className="text-[0.85rem] text-text mb-1 whitespace-pre-wrap">{r.description}</p>
                      {r.evidenceUrl && (
