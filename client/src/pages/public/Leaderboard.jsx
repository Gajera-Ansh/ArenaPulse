      ...
      <div className="bg-surface border border-border rounded-[8px] shadow-sm overflow-hidden">
        <div className="overflow-x-auto custom-scrollbar">
          <div className="min-w-[800px]">
            {/* Table Header */}
            <div className="grid grid-cols-12 gap-2 px-6 py-3.5 bg-bg border-b border-border text-[0.7rem] font-bold text-text-secondary uppercase tracking-[0.15em]">
            ...
            {!loading && activeTab === 'teams' && teamData.map((entry, index) => (
              <div
                key={entry.team._id}
                className={`grid grid-cols-12 gap-2 px-6 py-4 items-center border-b border-border/50 hover:bg-primary/[0.03] transition-colors ${getRankStyle(index)}`}
              >
                {/* Rank */}
            ...
            {!loading && activeTab === 'players' && playerData.map((entry, index) => (
              <Link
                ...
                className={`grid grid-cols-12 gap-2 px-6 py-4 items-center border-b border-border/50 hover:bg-primary/[0.03] transition-colors ${getRankStyle(index)} cursor-pointer`}
              >
                {/* Rank */}
                <div className="col-span-1 flex items-center justify-center">
                  {getRankIcon(index)}
